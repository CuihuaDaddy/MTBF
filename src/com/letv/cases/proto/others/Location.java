...
public class Location extends LetvTestCase {
	...
	public void openGPS() throws UiObjectNotFoundException {
		...
		.resourceId("android:id/decor_content_parent")
		.packageName("com.android.settings")
		.childSelector(
				new UiSelector().resourceId("android:id/content").className("android.widget.FrameLayout")));
		UiObject location = new UiObject(new UiSelector().className(
		...
		if (switchWidget.isChecked()) {
			...
			switchWidget.click();
			sleepInt(2);
			UiObject Agree = new UiObject(new UiSelector().resourceId(
					"android:id/button1").text("Agree"));
			Agree.click();
			sleepInt(2);
	...
	public void closeGPS() throws UiObjectNotFoundException {
		...
		if (switchWidget.isChecked()) {
			...
			sleepInt(2);
			UiObject connectInfo = new UiObject(new UiSelector().resourceId(
					"com.android.settings:id/switch_text").text("Off"));
			verify("Can't close Location.",
					!(switchWidget.isChecked()) || connectInfo.exists());
		} else {
			return;
		}
